...
void richgeo::IrtGeoDRICH::DD4hep_to_IRT() {
  ...
  auto filterMaterial   = m_det->constant<std::string>("DRICH_filter_material");
      // quartz window //RJ
    auto quartzwinZpos = m_det->constant<double>("DRICH_QW_zpos") / dd4hep::mm; 
    auto quartzwinThickness = m_det->constant<double>("DRICH_quartzwindow_thickness") / dd4hep::mm;
    auto quartzwinMaterial = m_det->constant<std::string>("DRICH_quartzwindow_material"); 
    auto quartzwinRmin = m_det->constant<double>("DRICH_quartzwindow_rmin") / dd4hep::mm; 
    auto quartzwinRmax = m_det->constant<double>("DRICH_quartzwindow_rmax") / dd4hep::mm; 
    auto quartzwinZmin = m_det->constant<double>("DRICH_quartzwindow_zmin") / dd4hep::mm;
    auto quartzwinZmax = m_det->constant<double>("DRICH_quartzwindow_zmax") / dd4hep::mm;


    double qw_slope = (quartzwinRmax - quartzwinRmin) / (quartzwinZmax - quartzwinZmin);
    double qw_angle = atan(qw_slope) - M_PI/2.0; 
    double quartzwinRmax2 = quartzwinRmax / cos(qw_angle); 
    dd4hep:: RotationY tiltRotation(qw_angle); 
...
//today 23/06/26
 ...
 //RJ
  ...
  dd4hep::Rotation3D rot = dd4hep::RotationZ((isec+0.5)*2.0*M_PI/nSectors)* tiltRotation;  //calcultes the rotation 
  TVector3 P_local = rot * TVector3(quartzwinRmax2,0,0); // 
  TVector3 origin(P_local.X(), P_local.Y(), quartzwinZpos + P_local.Z()); // both plots are showings, shift in Z
  ...
  auto *quartzwinFlatRadiator = m_irtDetectorCollection->AddFlatRadiator( 
         m_irtDetector,    // Cherenkov detector
         "QuartzWindow",  //Should we change QW name?
         isec,            // path
         (G4LogicalVolume*)(0x3), // G4LogicalVolume (inaccessible? use an integer instead)
         nullptr,   // G4RadiatorMaterial
         surface,  //Surface
         quartzwinThickness); //Surface thickness
  
    quartzwinFlatRadiator->SetAlternativeMaterialName(quartzwinMaterial.c_str()); 
  ...
  rIndices.insert({"QuartzWindow", 1.4572}); // SiO2 at ~400 nm //RJ
...
richgeo::IrtGeoDRICH::~IrtGeoDRICH() {
  ...
  delete m_quartzwinFlatSurface; //RJ
}
