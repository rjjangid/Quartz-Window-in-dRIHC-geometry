...
void IrtCherenkovParticleID::process(const IrtCherenkovParticleID::Input& input,
  ...
  auto* hist = irt_particle->FindRadiatorHistory(irt_rad);

  if (!hist) continue;

//  std::cout
 //   << "After PIDReconstruction : "
 //   << rad_name
 //   << " photons = "
 //   << hist->Photons().size()
 //   << std::endl;

  for (auto* phot : hist->Photons()) {

   // std::cout
    //  << " PDF size = "
     // << phot->_m_PDF.size()
      //<< " selected size = "
      //<< phot->_m_Selected.size()
      //<< std::endl;
 ...
 // << std::endl; //RJ
  ...
  //      std::cout
  //<< "Photon selected list size = "
 // << irt_photon->_m_Selected.size()
  //<< std::endl; //RJ
        if (!photon_selected) {
          continue;
        }
   ...
   //   std::cout
 // << "Created PID object for radiator "
 // << rad_name
 // << "  NPE=" << npe
//  << std::endl; //RJ
